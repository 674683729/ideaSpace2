...
import com.fh.shop.api.pay.result.PayResult;
        ...
        try {
            ...
            while(true) {
                ...
                if (trade_state.equalsIgnoreCase("SUCCESS")) {
                    ...
                    //清除redis中支付日志
                    RedisUtil.del(KeyUtil.buildPayLogInfo(memberId));
                    return ServerResponse.success(payLog.getPayPrice());
                }
                count++;
                Thread.sleep(3000);
                if (count >= 20) {
